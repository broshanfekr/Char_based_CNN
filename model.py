...
class CharCNN(object):
    ...
    def __init__(self, num_classes=2, filter_sizes=(7, 7, 3, 3, 3, 3), num_filters_per_size=256,
        ...
        reduced_features = sequence_max_length
        # ================ Layer 1 ================
        with tf.name_scope("conv-maxpool-1"):
            filter_shape = [num_quantized_chars, filter_sizes[0], 1, num_filters_per_size]
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W1")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b1")
            conv = tf.nn.conv2d(self.input_x, W, strides=[1, 1, 1, 1], padding="VALID", name="conv1")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")

            reduced_features = reduced_features - filter_sizes[0] + 1
        ...
        with tf.name_scope("conv-maxpool-2"):
            ...
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W2")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b2")
            conv = tf.nn.conv2d(pooled, W, strides=[1, 1, 1, 1], padding="VALID", name="conv2")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")

            reduced_features = reduced_features - filter_sizes[1] + 1
        ...
        with tf.name_scope("conv-3"):
            ...
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W3")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b3")
            conv = tf.nn.conv2d(pooled, W, strides=[1, 1, 1, 1], padding="VALID", name="conv3")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")

            reduced_features = reduced_features - filter_sizes[2] + 1

        # ================ Layer 4 ================
        with tf.name_scope("conv-4"):
            filter_shape = [1, filter_sizes[3], num_filters_per_size, num_filters_per_size]
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W4")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b4")
            conv = tf.nn.conv2d(h, W, strides=[1, 1, 1, 1], padding="VALID", name="conv4")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")

            reduced_features = reduced_features - filter_sizes[3] + 1

        # ================ Layer 5 ================
        with tf.name_scope("conv-5"):
            filter_shape = [1, filter_sizes[4], num_filters_per_size, num_filters_per_size]
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W5")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b5")
            conv = tf.nn.conv2d(h, W, strides=[1, 1, 1, 1], padding="VALID", name="conv5")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")

            reduced_features = reduced_features - filter_sizes[4] + 1

        # ================ Layer 6 ================
        with tf.name_scope("conv-maxpool-6"):
            filter_shape = [1, filter_sizes[5], num_filters_per_size, num_filters_per_size]
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W6")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b6")
            conv = tf.nn.conv2d(h, W, strides=[1, 1, 1, 1], padding="VALID", name="conv6")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")

            reduced_features = reduced_features - filter_sizes[5] + 1
            pooled = tf.nn.max_pool(
                h,
                ksize=[1, 1, 3, 1],
                strides=[1, 1, 3, 1],
        ...
        # ================ Layer 7 ================
        num_features_total = int(reduced_features) * num_filters_per_size
        ...
        with tf.name_scope("fc-2"):
            W = tf.Variable(tf.truncated_normal([1024, 1024], stddev=0.05), name="W7")
            # W = tf.get_variable("W", shape=[1024, 1024],
            #                     initializer=tf.contrib.layers.xavier_initializer())
            b = tf.Variable(tf.constant(0.1, shape=[1024]), name="b7")
            # l2_loss += tf.nn.l2_loss(W)
            # l2_loss += tf.nn.l2_loss(b)
        ...
        with tf.name_scope("fc-3"):
            W = tf.Variable(tf.truncated_normal([1024, num_classes], stddev=0.05), name="W8")
            b = tf.Variable(tf.constant(0.1, shape=[num_classes]), name="b8")
            l2_loss += tf.nn.l2_loss(W)
            l2_loss += tf.nn.l2_loss(b)
